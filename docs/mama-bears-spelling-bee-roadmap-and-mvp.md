...
- **`src/features/play/logic/bee-line.ts`**: `BeeLineScoreTuning` (`perCorrect`/`wrongOrderPenalty`/`wrongLetterPenalty`/`timeoutPenalty?`), `BeeLineScoreState` (`{ running: number }`), and the pure `applyScore(state, outcome, tuning)` — exactly the shape architecture 26.6 specified, appended to `bee-line.ts` alongside `resolvePickup` rather than in a separate file (the module's own header comment, previously stale — it claimed `applyScore` "does not live here" — was corrected in the same change). `applyScore` credits `perCorrect` on `'correct'`, subtracts `wrongOrderPenalty` on `'wrong-order'`, subtracts `wrongLetterPenalty` on `'wrong-letter'`, and never clamps at zero — the running total can and does go negative mid-attempt, exactly as 26.6 calls for.
- **`DEFAULT_BEE_LINE_SCORE_TUNING`** ships as `{ perCorrect: 10, wrongOrderPenalty: 5, wrongLetterPenalty: 15 }` — explicitly commented as a launch **placeholder only, not yet ratified by product**, mirroring how `DEFAULT_BEE_LINE_TUNING` is already flagged. The one locked constraint (`wrongLetterPenalty > wrongOrderPenalty`, since a wrong-letter mistake also scatters the trail) is satisfied and directly unit-tested; the exact magnitudes are free to change with zero other code changes.
- **`bee-line.test.ts`**: 10 new tests (`DEFAULT_BEE_LINE_SCORE_TUNING` weighting + a 9-test `applyScore` `describe` block) covering: crediting on `correct`; dipping on `wrong-order`/`wrong-letter`; `wrong-letter` dipping more than `wrong-order` from the same starting score; dipping below zero; purity (no mutation of the input state); and — per this epic's explicit task — a full simulated `easy`-tier attempt driven through the real `resolvePickup`, confirming the resulting score only ever moves by `perCorrect`/`wrongOrderPenalty` and never touches `wrongLetterPenalty`, since `easy`'s `decoyLetterCount: 0` makes `wrong-letter` structurally unreachable there (matching the identical guarantee `resolvePickup` itself already had a test for in Epic 18).
- **`BeeLineScreen.tsx` — running score UI**: a new `scoreState` (`useState<BeeLineScoreState>`), reset to `{ running: 0 }` in the same per-word `useEffect` that already resets `collectionState`/`headPathRef`/etc., fed by `applyScore` inside `attemptPickup` on every outcome. A plain `Score: N` readout now renders below the target-word card (there was previously no visible score UI at all in this screen) — entirely local component state, never touching `session-store.ts`'s `incrementScore`/`score` and never feeding `progress-store.ts`; `handleWordComplete`'s existing `recordWordCompleted(word)` call is completely unchanged and fires regardless of what the running score did during the attempt.
- **Floating "-N" popup**: a small local `Animated.Text` (no new reusable component — this repo's stated no-premature-abstraction convention, and `Confetti.tsx` was reviewed but didn't fit a single floating number), anchored via `position: 'relative'` on the score row, rising and fading over 700ms. Fires on both `wrong-order` and `wrong-letter` dips (the roadmap's task list named the "-N" popup explicitly for `wrong-order` and implied the same "bigger dip" treatment for `wrong-letter`; showing it for both was the natural reading) — not on `correct` pickups, which the task list never asked for.
- **`wrong-order` treatment**: `applyScore`'s dip, the floating popup, a quick tile wobble (rotate oscillation, `Animated.sequence`, ~320ms) plus a warm-orange (`#FF8C00`) outline flash held for ~380ms on the specific mis-tapped tile (tracked by id, applied identically across the tap-mode, drag-mode-passive, and drag-mode-draggable tile render paths so it works regardless of input tier), and the `bee-line-wrong-order` sound cue. `DEFAULT_BEE_LINE_TUNING.wrongOrderPolicy: 'keep-chain'` (Epic 18, untouched) already leaves the trail/collected state fully intact — the tile "bounces back" simply by never having moved.
- **`wrong-letter` treatment**: the bigger `applyScore` dip, the floating popup, a comedic "sproing/poof" scatter burst, a startled bee headshake (rotate oscillation on the 🐝 emoji — never a sad/scolding animation), the `bee-line-wrong-letter` sound cue, and the target word re-displaying via the existing `revealWord` banner (replayed TTS + the same auto-dismissing overlay a fresh word already uses) once the field rebuild lands — reusing an existing, precedented mechanism rather than inventing a new "try again" confirmation UI, and satisfying "no separate confirmation tap" since the banner still auto-dismisses on its own.
- **Fixed an explicitly-flagged Epic 19.5 gap while building the scatter burst**: Epic 19.5's own notes (and architecture 26.11) flagged that on a chain break, the trailing segments vanished *instantly* (they derive synchronously from `collectionState.nextExpectedIndex` resetting to 0) while the head visually froze until the delayed rebuild — a "trail gone, head lingers, then head jumps" look, explicitly deferred to this epic. Fixed via a `ScatterSnapshot`: the in-progress trail's positions/letters are captured into local state the instant a `wrong-letter` outcome is detected, *before* the pre-existing `setCollectionState(result.next)` call empties the live trail, and rendered as its own overlay (with the scatter transform) for the `CHAIN_BREAK_HOLD_MS` window; the live head element is hidden for that same window so the two don't double up.
- **`src/shared/lib/sound-effects/sound-effects-service.ts`** (new, `index.ts` barrel matching `character-audio`'s): `soundEffectsService.playCue(cue: SoundEffectCue)` with `SoundEffectCue = 'bee-line-wrong-order' | 'bee-line-wrong-letter'`, built by mirroring `characterAudioService` field-for-field (`createAudioPlayer`, try/catch around playback, `didJustFinish`-triggered cleanup, a shared `activePlayers` set, `stopAll()` called from `BeeLineScreen`'s existing unmount cleanup effect alongside `speechService.stop()`). **Judgment call, flagged explicitly**: the asset registry (`SOUND_EFFECTS`) ships **empty** — there are zero `.mp3`/`.wav`/`.m4a` files anywhere in this repo today, the same state Epic 7/9/11 shipped character voice lines in. `playCue()` fails silently (no-op, returns `false`) when a cue has no registered asset, so the wiring is fully in place and "just works" the moment real clips are dropped in and `require()`'d into the registry — zero further code changes needed. This is a new, narrower sibling to `characterAudioService` (game SFX, not per-character voice lines) rather than forcing "uh-uh!"/"whoa!" into `CharacterAudioVariant`, which has no generic "mistake" slot and is semantically about character identity, not game feedback.
- **Not touched, per this epic's explicit boundaries**: `session-store.ts`'s `score`/`incrementScore` (Bee Line's running score never calls it), `progress-store.ts`'s `recordCorrectFlick`/`recordMissedFlick`/`wordStats` (untouched; only the existing `recordWordCompleted` call fires, unchanged), and `resolvePickup`/`CollectionState`/`ChainPolicy`/`BeeLineTuning` (Epic 18's classifier survives a second epic untouched, extending its Epic 19.5 record).
- **A documentation gap surfaced, not silently resolved**: architecture 26.8 describes Mama Bear as "reused unchanged" for Bee Line, reacting to a wrong-order slip via the existing `useCharacterAnimationState`/`<Character>` path "the same way she responds to an HPF miss today." In the actual repo, `BeeLineScreen.tsx` has never imported or rendered `<Character>` at all — Epic 20's own task list doesn't ask for it either. This epic did not wire Mama Bear in (out of scope per the task list above), but the discrepancy between 26.8's "reused unchanged" framing and the screen's actual zero-Character-integration state is flagged here rather than quietly left implied-done.
- **Regression-verified**: `npm test` passes 94/94 (10 new tests in `bee-line.test.ts`; no regressions in the pre-existing 84). `npx tsc --noEmit` is clean. `npx expo export --platform web` builds successfully (2.16 MB web bundle, unchanged from Epic 19.5 — expected, no new dependencies). As with every prior Bee Line epic, no browser-automation/gesture-simulation tool is available in this environment, so the new score UI, popup, wobble/flash, scatter burst, and bee-headshake animations were verified by careful static/type review and the clean `tsc`/bundle pass, **not** a live playtest — stated plainly rather than claimed. The `wrong-letter` scatter/headshake path in particular remains unreachable in a live game today (every tier still ships `decoyLetterCount: 0` until Epic 21), same as `resolvePickup`'s `chainBroke: true` branch has been since Epic 18 — implemented and reviewed for correctness, not exercised end-to-end.

#### Open Items Carried Forward

- **Exact `perCorrect`/`wrongOrderPenalty`/`wrongLetterPenalty` values** remain placeholders (`10`/`5`/`15`) — product/UX to set real numbers once the mechanic is playable in front of a child, as the roadmap always intended.
- **Whether the running score persists as a per-session total anywhere** is still undecided — nothing here persists it; if that changes, it must land in a new, separately-named field/slice, never `session-store.ts`'s `score`.
- **Sound cue assets are not yet delivered** — `SOUND_EFFECTS` in `sound-effects-service.ts` is empty by design; wiring is complete and silent-safe until real `.mp3` files land.
- **No live-device/gesture playtest** of the `hard`+ scatter/wobble/headshake animations — same stated limitation as Epic 19/19.5, carried forward rather than re-litigated.
- **Mama Bear/villain reactive wiring for Bee Line (architecture 26.8)** remains entirely unbuilt — `BeeLineScreen.tsx` has no `<Character>` integration at all yet, contrary to 26.8's "reused unchanged" framing. Flagged for whichever future epic actually wires it in (not scoped to Epic 20's task list).
...
- **`src/features/play/logic/bee-line.ts` — `BEE_LINE_MODE_CONFIG`**: `crazy.decoyLetterCount` is `3`, `impossible.decoyLetterCount` is `5`, both explicitly commented as launch **placeholders, not yet ratified by product/UX** (mirroring how `DEFAULT_BEE_LINE_TUNING`/`DEFAULT_BEE_LINE_SCORE_TUNING` are already flagged). `easy`/`hard` stay at `0`, per this epic's own scope. Reasoning for the numbers: Honey Pot Flick's own `GAME_MODE_CONFIG.decoyLetterCount` ladder (`game-modes.ts`) is `0/3/6/8`, and this epic's instructions explicitly authorized Bee Line taking its own, smaller numbers rather than copying HPF's — `BeeLineScreen.tsx`'s field (`styles.field`, ~340×340px) is noticeably smaller than HPF's catching field, and `buildBeeLineField`'s `DEFAULT_MIN_TILE_SPACING_PX` (64px) between every tile pair, correct or decoy, eats into that space fast. Empirical probing done while picking these numbers (see the crowding note below) confirmed 3/5 keeps the field usable at the app's typical word lengths while still escalating crazy→impossible the same shape HPF uses. `impossible.randomizePositionsPerAttempt` is now `true`; the other three tiers stay `false` — the one non-placeholder, fully-specified part of this epic's task list.
- **Bug found and fixed — the chain-break rebuild silently violated "a retry reuses the same field" for `crazy`**: `BeeLineScreen.tsx`'s `attemptPickup`, in its `result.chainBroke` branch, unconditionally called `buildBeeLineField(field.word, modeConfig.decoyLetterCount, fieldBounds!)` with the default `Math.random()` after every chain break, regardless of tier. Because every tier shipped `decoyLetterCount: 0` before this epic, a `wrong-letter` outcome (the only outcome that can set `chainBroke: true` under the default `break-chain` policy — see `resolvePickup`) was structurally unreachable, so this branch was dead code — Epic 20's own doc addendum flagged it as "implemented for correctness... not exercised end-to-end." This epic is what makes `crazy`/`impossible` capable of ever producing a real decoy pickup, so the branch is reachable in a live game for the first time, and its actual behavior turned out to be wrong for every tier except `impossible`: calling `buildBeeLineField` again always re-rolls fresh random tile positions, which directly contradicts this epic's own task list ("a retry reuses the same field") for `crazy` (and `hard`, if it ever gets decoys — it doesn't in this epic). **Fix**: the rebuild is now gated on `modeConfig.randomizePositionsPerAttempt`. When `true` (`impossible` only), `buildBeeLineField` is called again, generating a fresh layout for the next attempt — exactly the "once per attempt" behavior architecture 26.4 calls for. When `false` (`easy`/`hard`/`crazy`), the existing `field` object is reused as-is with no rebuild call at all; a tile's on-field position and its "collected" status were already tracked separately (`collectionState.collected`, filtered against in `remainingTiles`), so calling the pre-existing `acknowledgeChainBreak(state)` against the unchanged `field` is sufficient to make every tile reappear at its original position — no new mechanism needed, just removing the incorrect unconditional rebuild call. Before the fix, a child retrying a `crazy`-tier word after a decoy mistake would have seen the whole field silently reshuffle, contradicting the "memorize the layout across retries" design intent UX Step 16 (and this epic's own task list) explicitly wanted preserved below `impossible`.
- **`BeeLineScreen.tsx` — field-build call sites**: the existing per-word effect (`useEffect` keyed on `[currentWord, fieldBounds]`) is untouched and already satisfied "once per word at easy/hard/crazy" for every tier, including `impossible`'s first attempt at a new word — no change was needed there. The chain-break handler above is the second, and only other, call site; together they satisfy "once per word at easy/hard/crazy, once per attempt at impossible" through the single `buildBeeLineField` function, exactly as architecture 26.4 specifies ("position randomization is just *when* you call the builder... one code path either way") — no second algorithm, no `impossible`-only branch inside `buildBeeLineField` itself.
- **Decoy rendering confirmed kind-agnostic, no code changes needed**: reviewed every tile render path in `BeeLineScreen.tsx` (tap-mode, drag-mode passive tiles, drag-mode draggable tiles, the head, the trailing segments) — none of them read `tile.kind` for any styling decision; every path renders through `<HexTile letter={tile.letter} size={TILE_SIZE} />` (trailing segments additionally pass a `backgroundColor`, but that's driven by trail-segment-vs-not, never by `kind`). UX Step 16's "no color-tell" requirement was already satisfied by the render layer's data shape before this epic (per Epic 19's own doc note) and remains true now that decoys are actually reachable in play — confirmed explicitly rather than assumed.
- **Replayable pronunciation control confirmed pre-existing, no second control built**: `handleMegaphonePress` (calls `speechService.speakWord(currentWord)`) and its `Pressable testID="megaphone-button"` inside `targetCard` already existed before this epic and render unconditionally at every tier, not just `crazy`+. This is a superset of the roadmap task's "at `crazy`+" ask, so it already satisfies the requirement as-is; nothing was added or duplicated.
- **A real field-density finding surfaced while writing this epic's spacing test, flagged rather than silently absorbed**: `buildBeeLineField`'s rejection-sampler (Epic 18) is documented as a graceful, non-throwing *best-effort* — it falls back to its last-attempted candidate if it can't clear `minTileSpacingPx` within `MAX_PLACEMENT_ATTEMPTS`, rather than guaranteeing the spacing invariant unconditionally. Empirical probing done while picking this epic's decoy counts found that on the real ~340×340px field at the real 64px default spacing, the sampler reliably (0 misses in 500 trials) seats up to ~9-10 tiles, but the miss rate climbs sharply from there (roughly 28% of trials missed the spacing target at 11 tiles, ~90%+ by 13-14) — independent of decoys specifically, just total tile count. The app's own built-in word lists (`src/data/word-lists/built-in-word-lists.ts`) include words up to 13 letters (e.g. `questionnaire`, `perseverance` at the age-10 tier) which, combined with even a modest decoy count, can push total tile count into that unreliable range. This is **not a new problem this epic introduces** — it's the already-flagged, still-open "field density/letter-tile size/reachability on the smallest device" item (architecture 26.11; also this epic's own Open Questions above, both explicitly carried as out-of-scope real-device work) — but decoys make it easier to hit in practice, since they add tiles on top of an already-tight budget for long words. Flagged here rather than picked a decoy number that papers over it without saying so. `bee-line.test.ts`'s new spacing test therefore deliberately uses a median-length word (`garden`, 6 letters, representative of the built-in lists' typical length) rather than a worst-case long word, so it verifies the real, common-case guarantee without being a flaky assertion of something the builder never actually promised for every possible word.
- **`bee-line.test.ts`**: 4 new tests added replacing the one stale Epic-18-era test that asserted every tier's `decoyLetterCount`/`randomizePositionsPerAttempt` was `0`/`false` (no longer true) — split into: timer/music still unset everywhere (unaffected by this epic); `easy`/`hard` still `0`; `crazy`/`impossible` both `> 0` with `impossible >= crazy`; and `randomizePositionsPerAttempt` `true` only at `impossible`. One further new test (using a small seeded `mulberry32` PRNG for reproducibility, since a constant-value RNG stub would place every tile at the same point) confirms the minimum-spacing invariant holds at the real `crazy`/`impossible` decoy counts, on the real field size, for a representative median-length word — see the crowding finding above for why a worst-case-length word was deliberately not used.
- **Regression-verified**: `npm test` passes 98/98 (4 new tests in `bee-line.test.ts`; no regressions in the pre-existing 94). `npx tsc --noEmit` is clean. `npx expo export --platform web` builds successfully (2.16 MB web bundle, unchanged from Epic 20 — expected, no new dependencies). As with every prior Bee Line epic, no browser-automation/gesture-simulation tool is available in this environment, so the on-screen decoy rendering, the megaphone control under real decoys, and — most importantly — the fixed per-attempt-vs-per-word field-rebuild logic were verified by static/type review, the unit tests above (including the chain-break fix's underlying `modeConfig.randomizePositionsPerAttempt` branch, exercised indirectly via the `BEE_LINE_MODE_CONFIG` tests and directly via code review of `attemptPickup`, since `attemptPickup` itself is component-internal and not separately unit-testable without a rendering harness this repo doesn't have), and the clean `tsc`/bundle pass — **not** a live playtest. Stated plainly rather than claimed, matching Epic 19.5's and Epic 20's own convention.

#### Open Items Carried Forward

- **Exact decoy-count values (`crazy: 3`, `impossible: 5`) remain placeholders**, unratified by product/UX — picked from the roadmap's own suggested "3-4"/"5-6" range with reasoning documented above, but a real playtest could move them either direction.
- **No live-device/gesture playtest** of decoys, the pronunciation control under decoys, or the fixed chain-break rebuild logic — same stated limitation as every prior Bee Line epic, carried forward rather than re-litigated.
- **Field density/reachability on the smallest supported device remains unresolved** (architecture 26.11) — this epic surfaced concrete numbers for how it manifests (see the crowding finding above) but did not change `buildBeeLineField`, tile size, or field dimensions to address it; that remains explicitly out of scope per this epic's own Open Questions.
- **Optional accessibility/assist toggle** highlighting the next correct letter after repeated wrong-letter mistakes — still undecided, not built.
...
- **`src/shared/lib/music/game-music-service.ts`** (new): a `GameMusicService` singleton, `gameMusicService`, mirroring `characterAudioService`/`soundEffectsService`/`speechService`'s single-shared-instance shape. `playLoop(cue: AcceleratingMusicCue): boolean` looks up `cue.trackId` in a track registry; if unregistered it returns `false` and no-ops (never throws). If registered, it first calls `stop()` (only one background loop ever plays at once — a deliberate difference from `characterAudioService`/`soundEffectsService`'s one-player-per-call SFX design, where overlapping playback is intentional), then creates an `expo-audio` `AudioPlayer` with `loop = true`, starts playback, and starts a `setInterval` that recomputes elapsed time since the loop started and calls `player.setPlaybackRate(resolvePlaybackRateAtElapsedMs(cue, elapsedMs))` on every tick. `stop()` clears that timer and pauses/removes the player; `stopAll()` is kept as a distinct method name — per this epic's own instructions — purely for call-site consistency with the other two services' unmount convention (`BeeLineScreen.tsx`'s existing unmount effect already calls `characterAudioService.stopAll()`/`speechService.stop()` side by side, and a future epic wiring music in will want the same shape to reach for), but its body is just `this.stop()`: this service only ever has one active loop, so there's no distinct "stop everything vs. stop the current thing" behavior to justify separate logic, only separate names for call-site symmetry. `createAudioPlayer`/`player.play()`/`player.setPlaybackRate()`/`player.pause()`/`player.remove()` are all wrapped in `try`/`catch`, matching `character-audio-service.ts`'s exact fail-silent discipline.
- **`AcceleratingMusicCue` reused, not redefined**: `game-music-service.ts` imports the type directly from `src/features/play/logic/bee-line.ts` (`import type { AcceleratingMusicCue } from '@/features/play/logic/bee-line';`) rather than declaring a second copy, the same cross-module reuse pattern `snake-trail.ts` already uses for `Point` (imported from `honey-pot-flick.ts`). One source of truth for the shape both the gameplay-tuning side (`bee-line.ts`'s `BEE_LINE_MODE_CONFIG.impossible.music`, Epic 23's job to populate) and the playback side (`game-music-service.ts`) agree on.
- **Rate-ramp math split into its own pure module**: `src/shared/lib/music/playback-rate-ramp.ts` exports `resolvePlaybackRateAtElapsedMs(cue, elapsedMs)` with zero `expo-audio`/timer imports, mirroring `sprite-animation.ts`'s pure-module-with-sibling-test pattern more literally than folding the math into `game-music-service.ts` itself and testing it from there — `sprite-animation.ts` is a standalone pure sibling of the Reanimated-dependent `useSpriteAnimation.ts`, not code embedded inside the RN-consuming file, so `playback-rate-ramp.ts` follows that same file boundary rather than the alternative (also-permitted) option of a single `game-music-service.test.ts` testing exported logic out of the service file. Linear interpolation from `startRate` to `endRate` over `rampMs`, clamped: `elapsedMs <= 0` → `startRate`; `elapsedMs >= rampMs` → `endRate` (pinned, never overshoots or wraps past the ramp's end); `rampMs <= 0` → an instant jump to `endRate` with no divide-by-zero. `playback-rate-ramp.test.ts` (6 new tests) covers all of these: start, a midpoint interpolation, exactly-at-`rampMs`, past-`rampMs`, a negative `elapsedMs`, and `rampMs <= 0`.
- **Rate-ramp timer interval: a plain 100ms `setInterval`**, not a Reanimated/`requestAnimationFrame`-style per-frame driver. Background music's perceptible "smoothness" bar is much coarser than a visual animation's — a rate step every 100ms is inaudible as a discrete jump — and this service has no rendering to synchronize with, so a plain timer avoids pulling any animation-driver machinery into an audio-only service. Documented inline as `RATE_TICK_INTERVAL_MS`.
- **Track registry: inlined directly in `game-music-service.ts`** as `const MUSIC_TRACKS: Partial<Record<string, number>> = {}`, rather than split into a sibling lookup file the way `character-audio.ts` is split from `character-audio-service.ts`. Reasoning: `character-audio.ts`'s split exists because it's a two-level lookup (per-character, then per-variant) worth naming and documenting on its own; this service's registry is a flat `trackId -> asset` map with no such grouping, the same shape `sound-effects-service.ts`'s inline `SOUND_EFFECTS` already uses in this exact codebase for the same reason — followed that closer precedent rather than the character one. Ships with zero entries registered, confirmed: `playLoop('any-unregistered-id', ...)` finds `undefined` in the empty map, returns `false`, and never calls `createAudioPlayer` — no throw, no bundler `require()` of a nonexistent asset file.
- **`index.ts` barrel**: `export { gameMusicService } from './game-music-service'; export { resolvePlaybackRateAtElapsedMs } from './playback-rate-ramp'; export type { AcceleratingMusicCue } from '@/features/play/logic/bee-line';` — re-exports the cue type from its actual source (`bee-line.ts`) rather than re-declaring it, keeping the "don't redefine it" decision visible at the barrel too.
- **`BeeLineScreen.tsx` untouched, by design**: this epic is standalone infrastructure per its own "Depends On: Nothing" — no screen wiring, no `BEE_LINE_MODE_CONFIG` changes. Epic 23 is what will call `gameMusicService.playLoop(...)` from the `impossible`-tier timer flow.
- **Regression-verified**: `npm test` passes 104/104 (6 new tests in `playback-rate-ramp.test.ts`; no regressions in the pre-existing 98). `npx tsc --noEmit` is clean. `npx expo export --platform web` builds successfully at 2.16 MB — byte-identical to Epic 21's bundle, confirming the "zero tracks registered, nothing wired into any screen" scope actually holds: since no screen imports `game-music-service.ts` yet, Metro doesn't even pull the new module into the web bundle graph. As with every prior audio-touching epic in this codebase, no live-device audio playtest is possible in this environment — stated plainly rather than claimed; verification here is `tsc`/test/bundle only.

#### Open Items Carried Forward

- **The music acceleration curve, and whether "accelerating" is a `playbackRate` ramp on one stem vs. a crossfade between pre-rendered tempo stems**, remains an open asset-pipeline call for UX/audio (architecture 26.7/26.11) — not decided by this epic, and both are expressible behind the unchanged `AcceleratingMusicCue` shape without further code changes to this service.
- **No real music track assets exist yet** — `MUSIC_TRACKS` ships empty, matching `character-audio.ts`/`sound-effects-service.ts`'s exact "plumbing before art" precedent. `playLoop` is fully wired and ready the moment a real `.mp3` is dropped in and registered.
- **No live-device audio playtest** of the loop, the rate ramp, or `stop()`/`stopAll()`'s actual audible behavior — same stated limitation every prior audio-touching epic in this codebase has disclosed (Epic 7/9/11's character voice lines, Epic 20's SFX cues), carried forward rather than re-litigated; this environment has no audio-capable device or emulator to verify against.
...
- **`src/features/play/logic/bee-line.ts` — `computeTimeBudgetMs`**: `computeTimeBudgetMs(wordLength, cfg)` implements the exact shape the brief confirmed: `clamp(wordLength * cfg.secondsPerLetter * 1000, cfg.floorMs, cfg.ceilingMs)`, written as `Math.min(Math.max(Math.max(wordLength, 0) * cfg.secondsPerLetter * 1000, cfg.floorMs), cfg.ceilingMs)`. The inner `Math.max(wordLength, 0)` is the only departure from the bare formula — defensive against a zero/negative `wordLength` (never produced by a real word from a word list, but the roadmap's own task explicitly asked for the clamp to be unit-tested against it), and it composes cleanly with the floor/ceiling clamp already there rather than needing a separate branch. Pure, no RN/timer imports, sits directly beside the two type definitions it operates on, exactly where architecture 26.7's own code sketch places it.
- **`BEE_LINE_MODE_CONFIG.impossible.timer`**: `{ secondsPerLetter: 1.5, floorMs: 4000, ceilingMs: 15000 }`, flagged explicitly as an unratified launch placeholder, same convention as `DEFAULT_BEE_LINE_TUNING`/`DEFAULT_BEE_LINE_SCORE_TUNING`/Epic 21's decoy counts. Reasoning documented inline: `1.5s`/letter puts the built-in lists' median word length (6 letters, per Epic 21's own crowding-note finding) at 7.5-9s — a real race for the "confident 9-year-old" this tier's user story targets, without being unwinnable; `floorMs: 4000` keeps a very short word from being an instant-fail; `ceilingMs: 15000` caps the longest built-in words (up to 13 letters, which the bare formula would otherwise put at ~19.5s) at a number that keeps the "race against the clock" feel intact rather than drifting into an untimed-feeling long budget. `easy`/`hard`/`crazy` all keep `timer: undefined` — nothing changed there, `crazy` stays clock-free per UX Step 17's explicit instruction, now directly asserted in `bee-line.test.ts`.
- **`BEE_LINE_MODE_CONFIG.impossible.music`**: `{ trackId: 'bee-line-impossible-chase', loop: true, startRate: 1.0, endRate: 1.4, rampMs: 15000 }`. `startRate`/`endRate` mirror architecture 26.7's own "e.g. 1.0 -> ~1.4" example directly. The static `rampMs` here (set to `ceilingMs`, the longest a real ramp could ever run) is a required-by-type filler only — documented inline in the config and restated here per the brief's explicit ask — because every `gameMusicService.playLoop(...)` call site in `BeeLineScreen.tsx` spreads this cue and overrides `rampMs` with that attempt's real `computeTimeBudgetMs(...)` result, so the actual acceleration curve always peaks exactly at that word's real deadline, never at this placeholder value.
- **`bee-line.test.ts`**: 7 new tests. A `computeTimeBudgetMs` `describe` block covers: a short word (`wordLength: 2`) clamping to `floorMs`; a long word (`13`, the built-in lists' longest) clamping to `ceilingMs`; a mid-length word (`6`) landing exactly at the formula's unclamped output (`9000`), verifying the concrete number rather than just "in range"; a longer word producing a strictly larger budget than a shorter one; zero and negative `wordLength` both clamping to `floorMs` with no `NaN`; and purity (repeated calls, same output). The existing `BEE_LINE_MODE_CONFIG` `timer`/`music` test was split into two: one confirming every tier except `impossible` still has both `undefined`, and a new one confirming `impossible` has both defined while `crazy` stays `undefined` on both — restating UX Step 17's "don't let anything leak below impossible" guarantee directly, the same way Epic 21 restated it for decoys/position-randomization.
- **Fuse/sparkler UI (`BeeLineScreen.tsx`)**: a small horizontal bar (`FUSE_BAR_WIDTH_PX`/`FUSE_BAR_HEIGHT_PX` = 84x10px) rendered above the steered head's current position, gated on `modeConfig.timer` being defined (so it only ever exists at `impossible`) and on no scatter/firework burst being in progress. The burn-down is a native-driver-compatible transform, not an animated `width`: the fill's left edge is pinned via a `scaleX`+compensating `translateX` pair driven by a single `fuseAnim` (`Animated.Value`, `1 -> 0` linearly over the attempt's computed budget), so the bar visibly shrinks from a fixed left edge rather than shrinking toward its center. A small "✨" spark rides the tip of the shrinking fill via its own `translateX` interpolation, giving it the "sparkler" read the roadmap named, not just a plain progress bar. A small numeral (`{secondsRemaining}s`, ~11px, `fuseSecondsText`) renders below the bar — confirmed secondary both visually (smaller, plain text, no border/background) and in code (the fuse bar is the unconditional element; the numeral is a further-nested conditional inside it) — satisfying "the fuse is the primary read, not the numeral" exactly as the brief locked in.
- **"Start a fresh timed attempt" consolidation — built, as suggested**: `startTimedAttempt(word)` is the single function all three trigger sites call. It clears any stale timer/interval/fuse-animation via a small `clearImpossibleTimer()` helper, then (only if `modeConfig.timer` is defined) computes the budget, resets and restarts the fuse animation, arms the `setTimeout` that calls `handleTimeout`, starts the seconds-tick `setInterval`, and — only if `modeConfig.music` is defined — calls `gameMusicService.playLoop({ ...modeConfig.music, rampMs: budgetMs })`. The three call sites: (1) the existing per-word field-build `useEffect` (`[currentWord, fieldBounds]`), right after `buildBeeLineField`; (2) `attemptPickup`'s `chainBroke` branch's existing rebuild `setTimeout`, right after its existing `revealWord(nextField.word)` call; (3) `handleTimeout`'s own rebuild `setTimeout`, in the same position. This exactly mirrors how `buildBeeLineField` already has two call sites sharing one function (Epic 21) — this is a third call site for a second per-attempt concern (timer+music) getting the same treatment, per the brief's own suggestion.
- **Cancellation**: `handleWordComplete` now calls `clearImpossibleTimer()` and `gameMusicService.stop()` as its first two lines (both harmless no-ops below `impossible`, since nothing was ever armed there), so a completed word can never let a stale timeout fire afterward — satisfying the task's explicit cancellation requirement. The screen's existing unmount cleanup `useEffect` (which already calls `speechService.stop()`/`soundEffectsService.stopAll()`) now also calls `clearImpossibleTimer()` and `gameMusicService.stopAll()`.
- **Music wiring**: confirmed exactly as specified — `playLoop`'s `rampMs` argument is always the per-attempt `computeTimeBudgetMs(...)` result, never `modeConfig.music`'s static placeholder, at all three `startTimedAttempt` call sites. `gameMusicService.stop()` is called at the two documented moments (immediately when a timeout fires, and in `handleWordComplete` for both a win and — implicitly, since `stop()` is a safe no-op — every tier). Since `MUSIC_TRACKS` in `game-music-service.ts` ships empty (Epic 22), every `playLoop` call here silently no-ops today, exactly as expected — "plumbing before art," the same precedent every prior audio-touching epic in this codebase has followed.
- **Timeout handling — `handleTimeout(word)`**: guarded identically to every other mutation path in this screen (`resolvingRef.current` / `collectionState.status !== 'in-progress'`), so a timeout that happens to land during another in-flight resolution (e.g. the ~450ms `CHAIN_BREAK_HOLD_MS` window right after a decoy mistake) safely no-ops rather than double-firing — the pending `setTimeout` that scheduled it is simply "spent," and the chain-break retry's own `startTimedAttempt` call arms a completely fresh timer for the next attempt regardless. On a real fire: captures a `ScatterSnapshot`-shaped `fireworkSnapshot` from the live trail (same capture-before-reset technique the wrong-letter path already uses — reused directly rather than duplicated, per the brief's explicit steer), calls `gameMusicService.stop()`, plays the new `bee-line-timeout` sound cue, and triggers a new, distinct burst animation (below). After `TIMEOUT_HOLD_MS` (`FIREWORK_BURST_DURATION_MS` + a 150ms buffer — deliberately longer than the wrong-letter scatter's `CHAIN_BREAK_HOLD_MS`, since the roadmap calls for this to read as "the bigger, firework-scale burst" on the shared comedic ladder), the word restarts fully from scratch: `buildBeeLineField` (unconditional here, since this path is impossible-only by construction and impossible always randomizes anyway), a full `createCollectionState(word)` reset (not the chain-break path's partial `acknowledgeChainBreak`, per the task's explicit "restart the same word from the beginning"), the usual ref/state clears, `revealWord(word)` (the same "try again" cue the chain-break retry already uses — judgment call: replay it for consistency between the two retry paths, rather than inventing a second confirmation treatment, since the brief left this an explicit "your call"), and a fresh `startTimedAttempt(word)` to arm the next attempt's timer/fuse/music.
- **The firework burst — new, distinct from `triggerScatterBurst`**: a new `triggerFireworkBurst()` drives a single new `fireworkAnim` (`0 -> 1` over `FIREWORK_BURST_DURATION_MS`, 650ms — longer than the scatter's 450ms). Two visually separate things happen under it: (1) the caught-mid-attempt trail/head tiles simply fade+shrink in place (opacity + scale only, no fling/rotate/translate) — deliberately the opposite treatment from the scatter's per-tile outward fling-and-spin, so the two consequences don't share a silhouette; (2) a fixed `FIREWORK_PARTICLES` table (10 small colored dots, computed once at module scope from a simple trig fan, not a reusable particle-system abstraction — this codebase's stated no-premature-abstraction convention, matching how the existing scatter is itself just a handful of `Animated.View`s) launches from the pre-timeout head position in a fan spanning roughly +/-75deg around straight up, alternating `theme.colors.gold`/`theme.colors.accent` (`#FF1493`, the closest "hot pink" token, reused rather than inventing a third one-off hex), fading and scaling as they travel. The net read is a single upward "whoosh" of bright particles from a fixed point, which is the opposite silhouette from `Confetti.tsx`'s falling-from-above shower and from the scatter's outward/downward fling — satisfying the roadmap's explicit "so a child can tell success from timeout within a second" requirement by construction, not just by color.
- **New `'bee-line-timeout'` sound cue**: added to `SoundEffectCue`'s union in `src/shared/lib/sound-effects/sound-effects-service.ts` (now `'bee-line-wrong-order' | 'bee-line-wrong-letter' | 'bee-line-timeout'`), a small additive edit exactly as scoped — no other change to that file's structure, registry, or fail-silent `playCue`/`stopAll` logic. `SOUND_EFFECTS` stays empty (no `.mp3` assets exist anywhere in this repo), so `playCue('bee-line-timeout')` silently no-ops today, matching the other two cues' exact state since Epic 20.
- **Score on timeout — judgment call, option (a) chosen**: **no score change on timeout.** `timeoutPenalty` (already present on `BeeLineScoreTuning` since Epic 20, explicitly commented `// impossible-tier timeout, if it dips score`) is left completely unwired — `handleTimeout` never calls `applyScore` and never touches `scoreState` directly. Reasoning: the field is optional (`?`) and was only ever "if it dips score," never a committed decision; the firework explosion itself, plus the full word restart, is already a real, visible, felt consequence — UX Step 18's comedic ladder treats it as the biggest *staging* beat, not necessarily the biggest *scoring* penalty, and the roadmap's own task list for this epic never explicitly asks for a score dip (unlike Epic 20's wrong-order/wrong-letter tasks, which named the score dip outright). Adding an unrequested scoring rule here would also be new, untested product behavior invented on this epic's own authority rather than the architecture's, which architecture 26.7 itself flags as still-open tuning, not a decided number. Option (b) (a direct `scoreState` dip bypassing `applyScore`) was considered and rejected for now, precisely because it would require inventing a magnitude with zero product guidance beyond "if it dips score" — better to ship the simpler, honest state (dedicated Open Item below) than guess a number and imply it's decided.
- **Villain reaction — confirmed out of scope, not touched**: `BeeLineScreen.tsx` still has zero `<Character>`/villain rendering, unchanged from every prior addendum's note (Epic 20/21/22). Nothing in this epic's diff references `useCharacterAnimationState`, `villain-behavior.ts`, or any character import — explicitly deferred to Epic 24 per this epic's own Open Questions and `Depends On`.
- **Not touched, per this epic's explicit boundaries**: `resolvePickup`, `CollectionState`, `ChainPolicy`, `BeeLineTuning`, `CollectionOutcome`, `applyScore`'s signature, and every existing field on `BeeLineScoreTuning` — Epic 18/20's classifier and scoring surface now survive a fourth epic untouched. `CollectionOutcome` was deliberately NOT extended with a `'timeout'` variant (per this epic's own explicit instruction): timer expiry is handled entirely as a screen-level flow (`handleTimeout`), never routed through `resolvePickup`, since a timeout isn't a *pickup* classification at all.
- **Regression-verified**: `npm test` passes 111/111 (7 new tests in `bee-line.test.ts`; no regressions in the pre-existing 104). `npx tsc --noEmit` is clean. `npx expo export --platform web` builds successfully (2.17 MB web bundle, up from Epic 21/22's 2.16 MB — expected and confirms the wiring is real: `BeeLineScreen.tsx` now imports `gameMusicService`/`computeTimeBudgetMs` for the first time, so Metro pulls `src/shared/lib/music/` into the bundle graph, exactly mirroring Epic 22's own note that it wouldn't be pulled in until a screen actually imported it). As with every prior Bee Line epic, no browser-automation/gesture-simulation tool is available in this environment, so the fuse rendering, the firework burst's visual distinctness, the full timer -> timeout -> restart cycle, and the music/sound wiring were verified by static/type review, the `computeTimeBudgetMs` unit tests, and the clean `tsc`/bundle pass — **not** a live playtest. Stated plainly rather than claimed, matching every prior epic's own convention.

#### Open Items Carried Forward

- **Exact `secondsPerLetter`/`floorMs`/`ceilingMs` (`1.5`/`4000`/`15000`) and the music cue's `startRate`/`endRate`/`trackId` remain placeholders**, unratified by product/UX — reasoning documented above, but a real playtest with a child in the target age band could move any of them.
- **No live-device/gesture/audio playtest** of the fuse burn-down, the firework burst, the music ramp, or the full timer->timeout->restart cycle — same stated limitation as every prior Bee Line/audio epic, carried forward rather than re-litigated.
- **Villain reaction to the timeout explosion remains entirely deferred to Epic 24**, per this epic's own Open Questions — `BeeLineScreen.tsx` still has zero `<Character>` integration.
- **`timeoutPenalty` on `BeeLineScoreTuning` remains unused** — still wired for a future epic to pick up if/when product decides the timeout should also dip score; this epic deliberately did not give it a value or a call site (see the score-on-timeout judgment call above).
- **The fuse's fixed vertical offset (`-34px` above the head) can clip against the field's `overflow: 'hidden'` top edge** when the head is near the field's top boundary — the same pre-existing risk `beeRider`'s own `top: -18` offset already carries (unaddressed since Epic 19), not a new problem this epic introduces, but worth a real-device check alongside architecture 26.11's already-open "field density/reachability" item.
- **No real audio assets exist for the new `bee-line-timeout` cue or the `bee-line-impossible-chase` track** — both are fully wired and fail-silent, ready the moment real clips/tracks are dropped in and registered, matching every other "plumbing before art" cue/track in this codebase.
...
- **The Objective/architecture's "mirrors how she already responds gently to a Honey Pot Flick miss today" claim is factually wrong — flagged, not silently worked around.** A grep of every `mamaBear.trigger(...)` call site in `PlayScreen.tsx` found exactly three: a tap gesture (`'Poked'`), word completion (`'Celebrating'`), and each correct-letter catch inside `resolveFlick` (`'Talking'`) — plus a separate, non-trigger animation-budget dev-check effect the brief also correctly anticipated wouldn't count. (One correction to the brief's own supporting research, confirmed by reading the actual call site: it's a *correct*-catch cheer inside `resolveFlick`, not the word-reveal banner — `revealWord` itself never calls `mamaBear.trigger` at all.) `rejectPot` — HPF's shared miss handler for both the off-target flick and the wrong-letter branches — calls only `villain.trigger('Challenging')` and opens a steal attempt; it never touches `mamaBear`. So there is nothing to literally "mirror" here; HPF's Mama Bear simply doesn't react to a miss today. Resolution: Bee Line's `wrong-order` reaction is implemented as new, reasonable behavior rather than a port of nonexistent code — `mamaBear.trigger('Talking')`, chosen because it's already this codebase's "warm, low-stakes cheer" beat (HPF's own per-correct-letter reaction) and reads warmer than `'Poked'`'s "playful bump." This matches the exact precedent this codebase has set for a doc/code mismatch (Epic 20's flagged "Mama Bear reused unchanged" discrepancy, Epic 19's "Professor Owl" naming fix, Epic 21's field-density finding) — say so plainly, don't quietly invent a match that isn't there.
- **Villain selection: local component state, not `session-store.ts`.** `BeeLineScreen.tsx` picks `villainId` once per mount via a `useState` lazy initializer calling `pickNextVillain(getEligibleVillains(VILLAIN_POOL_CONFIG, gameMode), null)` directly — the same pure primitives `session-store.ts`'s `pickSessionVillain` calls internally, but never touching that store. Reasoning: this screen has never imported `session-store.ts` at any point across Epics 18-23, and every prior Bee Line epic has kept this game's own state (running score, collection state, timer/fuse state) entirely local rather than reusing HPF's session store, even where the underlying primitive is shared (architecture 26.9's own "running score in local/Bee-Line state vs. reusing `session-store.score`" precedent argues the identical trade-off). The one real consequence of this choice: Bee Line's villain pick has no memory of the previous pick (`lastVillainId` is always passed as `null`), so unlike HPF it can repeat the same villain from a prior mount, and it does *not* get the cross-game "don't repeat whichever game played last" behavior that reusing the shared store's `lastVillainId` would have produced. Neither doc asked for that cross-game linkage, so this is the conservative, lower-coupling reading of the brief's genuinely ambiguous "reuse `pickNextVillain`/`villain-pool.ts` exactly as `PlayScreen.tsx` already does" instruction — the pure functions are reused exactly; the store is not.
- **New `src/features/play/logic/bee-line-villain-capabilities.ts`**: `BEE_LINE_VILLAIN_CAPABILITIES`, a `Record<VillainBehaviorTier, VillainCapability[]>` mirroring `villain-capabilities.ts`'s exact shape — `Passive`/`Taunting` both `[]`, `Interfering`/`Relentless` both `['Taunt']`. `getBeeLineVillainCapabilities(tier)` mirrors `getVillainCapabilities`'s signature. No tier lists `'StealResource'` — confirmed by a dedicated test (`bee-line-villain-capabilities.test.ts`, 3 new tests) that loops every tier and asserts `!capabilities.includes('StealResource')`, not just an implicit absence. `villain-behavior.ts` and `villain-capabilities.ts` (HPF's own table) are both untouched.
- **Villain visibility derived from capabilities, not a tier-string check**: `villainPresent = villainId != null && getBeeLineVillainCapabilities(getVillainBehaviorTier(gameMode)).length > 0`. This continues the screen's existing documented convention ("this screen needs no tier-specific branching beyond reading modeConfig") — there is no `gameMode === 'crazy' || gameMode === 'impossible'` anywhere in the diff; "villain absent at Passive/Taunting" and "the capabilities array is empty there" are the same fact expressed once.
- **The three reaction wire-ups (plus the timeout laugh), and exactly which `CharacterAnimationState` each uses**:
  1. **Word completion** (`triggerCelebration`, called from `handleWordComplete`): `mamaBear.trigger('Celebrating', CELEBRATION_TOTAL_MS)` unconditionally, paired with `villain.trigger('Defeated', CELEBRATION_TOTAL_MS)` when `villainPresent` — mirroring HPF's own `triggerCelebration` pairing (Celebrating/Defeated, held exactly as long as the confetti overlay) rather than inventing a different duration convention.
  2. **`wrong-order` mistake** (`attemptPickup`'s `wrong-order` branch, alongside the existing tile-wobble/sound): `mamaBear.trigger('Talking')` — see the corrected-doc reasoning above. Deliberately **not** wired on `wrong-letter`, per the roadmap task's explicit scoping (that outcome already gets the bigger bee-headshake/scatter treatment from Epic 20; layering Mama Bear on top wasn't asked for).
  3. **Villain heckle on any mistake** (`attemptPickup`, fired once at the shared point immediately after `triggerScorePopup`, upstream of both the `wrong-order` and `wrong-letter` `if` blocks — mirroring HPF's `rejectPot` "wired once, at the shared handler" precedent exactly): `villain.trigger('Challenging')`, gated on `villainPresent`.
  4. **Optional villain laugh on the `impossible` timeout** (`handleTimeout`, immediately after `triggerFireworkBurst()`/the timeout sound cue): `villain.trigger('Challenging')` again — the same heckle state, reused rather than invented, per architecture 26.8 option (a)'s scoping of `Challenging` to "the villain's heckle" generally. Fired as its own independent call with no positional link or shared `Animated.Value`/timeline with the firework particles, and the villain's row position never moves — satisfying the task's explicit "never staged as though the villain caused the explosion" instruction by construction, not just by timing.
- **No lunge, no tap gestures, no `BeingNaughty`.** The villain's `<Character>` renders at a fixed position inside `styles.characterRow` (a direct copy of HPF's `characterRow` style block); only its `animationState` prop changes. No `Animated.Value`-driven transform, no `GestureDetector`/`Gesture.Tap()` on either character — neither was asked for by this epic's task list, and adding them would have been unrequested scope.
- **Zero `StealAttemptHost`/`honeyStash`/steal-machinery references anywhere in this epic's diff** — confirmed by grep across the full diff: the only two string matches for `honeyStash`/`session-store` are inside explanatory code comments documenting *why* `session-store.ts` is deliberately not imported, not an actual import or call. `stealHoneyPot`, `earnHoneyPot`, `resetHoneyStash`, `incrementScore`, `resetScore`, and `session-store.ts`'s own `currentWord` are never referenced — Bee Line's pre-existing local `currentWord` state is untouched and remains the only `currentWord` this screen reads.
- **Reduce-motion**: no manual `useReduceMotion()` call was added in `BeeLineScreen.tsx`. Confirmed by reading `Character.tsx` in full: it already calls `useReduceMotion()` internally and skips the animated-sprite tier entirely when reduce-motion is on, falling back to the correct static pose image while still switching poses on every `animationState` change (its own documented "still switching poses on state changes" contract). The task's "gate any villain reaction animation through `useReduceMotion()`" instruction is only load-bearing for a *bespoke* `Animated` transform layered on top of a `<Character>` (exactly what HPF's villain lunge does, via its own `villainLungeAnim` + `runStealTravel`, which checks `reduceMotionEnabled` before animating) — and this epic explicitly forbids that exact lunge for Bee Line. Since the villain here is a bare `<Character animationState={...}>` with no bespoke transform, `Character.tsx`'s own internal gating is sufficient and no additional manual check was needed.
- **Not touched, confirmed**: `villain-behavior.ts`, `villain-capabilities.ts` (HPF's table), `character-animation-state.ts` (no new `CharacterAnimationState` enum value), `resolvePickup`, `CollectionState`, `ChainPolicy`, `BeeLineTuning`, `CollectionOutcome`, `applyScore`, `BeeLineScoreTuning`, `computeTimeBudgetMs`, `BeeLineTimerConfig` — a fifth consecutive Bee Line epic leaving this pure-logic surface untouched.
- **Regression-verified**: `npm test` passes 114/114 (3 new tests in `bee-line-villain-capabilities.test.ts`; no regressions in the pre-existing 111). `npx tsc --noEmit` is clean. `npx expo export --platform web` builds successfully (2.17 MB, unchanged from Epic 23's bundle — expected, since `Character`/`useCharacterAnimationState`/`villain-pool`/`villain-behavior` were already pulled into the shared web bundle graph via `PlayScreen.tsx`'s existing imports, so `BeeLineScreen.tsx` importing them for the first time added no new module weight). As with every prior Bee Line epic, no browser-automation/gesture-simulation tool is available in this environment — the character rendering, the villain-visibility gating, and all four reaction wire-ups were verified by static/type review, the new capabilities-map unit tests, and the clean `tsc`/bundle pass, **not** a live playtest. Stated plainly rather than claimed.

#### Open Items Carried Forward

- **Whether `Challenging` reads clearly as "heckling commentary" on real art, or a dedicated `Heckling` `CharacterAnimationState` value is warranted**, remains genuinely open — this epic implements option (a) only (zero contract change); UX to confirm the read once real art/animation exists, per architecture 26.8/26.11.
- **Whether villains ever get an active mechanic in Bee Line** remains explicitly deferred, not ruled out, per UX Step 19 — revisit only after the base mistake/timer tone (Epics 20/23) and this epic's staging have been playtested with real kids.
- **No live-device/gesture playtest** of the character row, the villain-visibility gating, or any of the four reaction wire-ups — same stated limitation as every prior Bee Line epic in this environment.
- **The corrected doc-accuracy note above should be trusted over the Objective/architecture text preceding it in this document**: Mama Bear does **not** react to a Honey Pot Flick miss in that game's actual shipped code today (confirmed by a full read of `PlayScreen.tsx`'s `rejectPot`), contrary to what this epic's own Objective and architecture 26.8 both claim. A future reader should not re-trust that specific claim without re-verifying against the live code.
